...
@section('content')
    <div class="container mx-auto px-6 py-8">
        <h3 class="text-gray-700 text-3xl font-medium">
            {{ isset($user) ? "Редактировать пользователя ID {$user->id}" : 'Добавить пользователя' }}</h3>
        ...
        <div class="mt-8">
            <form enctype="multipart/form-data" class="space-y-5 mt-5" method="POST"
                action="{{ isset($user) ? route('admin.admin_users.update', $user->id) : route('admin.admin_users.store') }}">
                @csrf
